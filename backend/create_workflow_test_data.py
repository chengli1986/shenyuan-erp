...
def create_workflow_test_data():
    ...
    try:
        ...
        print(f"📊 找到用户: {[k for k, v in users.items() if v]}")
            ...
            },
            {
                ...
                "workflow_logs": [
                    ...
                    },
                    {
                        ...
                        "operator": users['general_manager'],
                        "notes": "最终审批通过，授权采购执行",
                        "operation_data": {
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            if not scenario['requester']:
                print(f"   ⚠️  跳过: 找不到申请人用户")
                continue
            ...
            # 生成申购单编码
            request_code = f"WF-{datetime.now().strftime('%Y%m%d')}-{i+1:03d}"
            ...
            for j, item_data in enumerate(scenario['items']):
                item = PurchaseRequestItem(
                    ...
                    total_price=Decimal(str(item_data.get('unit_price', 0))) * Decimal(str(item_data['quantity'])) if item_data.get('unit_price') else None,
            ...
            for k, log_data in enumerate(scenario['workflow_logs']):
                if not log_data['operator']:
                    print(f"      ⚠️  跳过日志: 操作者不存在")
                    continue
                ...
                workflow_log = WorkflowLog(
                    ...
                    from_step=log_data['from_step'] if log_data['from_step'] else None,
